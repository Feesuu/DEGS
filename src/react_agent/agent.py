...
class _StagnationRecoveryExhausted(RuntimeError):
    """Internal signal that the bounded stagnation recovery was ignored."""


COMPLETION_RECOVERY_PROMPT = """Your previous response reached the completion limit. Continue exactly where it stopped without repeating the analysis or prior text. Finish the same response: complete the Action protocol message if one was in progress, otherwise complete the direct answer."""
...
class AgentConfig:
    ...
    stagnation_recovery_attempt_limit: int = 1
    """Number of blocked-action recovery prompts allowed per stagnation episode."""

    task_specific_stagnation_prompt: str = ""
    """Optional adapter-owned guidance appended to the generic stagnation prompt."""

    max_completion_tokens: int | None = None
    """Maximum generated tokens for each individual model request."""

    completion_recovery_attempt_limit: int = 1
    """Continuation requests allowed after a length-limited completion."""

    max_consecutive_format_errors: int = 2
    """Consecutive malformed model responses allowed before terminal failure."""

    truncate_observations: bool = True
    """Whether tool observations may be shortened before entering chat history."""
...
class ReActAgent:
    ...
    def __init__(
        ...
        if self.config.stagnation_recovery_attempt_limit < 0:
            raise ValueError("stagnation_recovery_attempt_limit must be non-negative")
        if (
            self.config.max_completion_tokens is not None
            and self.config.max_completion_tokens <= 0
        ):
            raise ValueError("max_completion_tokens must be positive when set")
        if self.config.completion_recovery_attempt_limit < 0:
            raise ValueError("completion_recovery_attempt_limit must be non-negative")
        if self.config.max_consecutive_format_errors < 1:
        ...
        self._stagnation_recovery_attempts = 0
    ...
    async def _chat_with_completion_recovery(
        self,
        messages: list[Message],
        settings: ModelSettings,
    ) -> str:
        """Recover one truncated protocol response without hiding terminal failure."""
        recovery_attempts = 0
        accumulated_partial = ""
        while True:
            try:
                ...
                if accumulated_partial:
                    parsed_response = self.converter.parse_response(response)
                    explicit_completion = any(
                        line.strip() == TASK_COMPLETE_SIGNAL
                        for line in response.splitlines()
                    )
                    if parsed_response.is_action or explicit_completion:
                        return response
                    return f"{accumulated_partial}{response}"
                return response
            except RequestCompletionLengthExceeded as exc:
                raw_partial = exc.partial_content
                partial = raw_partial.strip()
                accumulated_partial = f"{accumulated_partial}{raw_partial}"
                parsed_partial = self.converter.parse_response(partial) if partial else None
                explicit_completion = any(
                    line.strip() == TASK_COMPLETE_SIGNAL
                    for line in partial.splitlines()
                )
                if partial and (
                    (parsed_partial is not None and parsed_partial.is_action)
                    or explicit_completion
                ):
                    record_runtime_event(
                        "completion_limit_partial_response_accepted",
                        instance_id=self._runtime_instance_id,
                        protocol_sha256=self._runtime_protocol_sha256,
                        recovery_attempts=recovery_attempts,
                    )
                    return accumulated_partial
                if recovery_attempts >= self.config.completion_recovery_attempt_limit:
                    if raw_partial:
                        messages.append(
                            Message(role="assistant", content=_strip_think(raw_partial))
                        )
                    raise RequestCompletionLengthExceeded(
                        str(exc),
                        partial_content=accumulated_partial,
                ...
                if raw_partial:
                    ...
                    messages.append(
                        Message(role="user", content=COMPLETION_RECOVERY_PROMPT)
                    )
                ...
                record_runtime_event(
                    "completion_limit_recovery",
                    instance_id=self._runtime_instance_id,
                    protocol_sha256=self._runtime_protocol_sha256,
                    recovery_attempt=recovery_attempts,
                    recovery_limit=self.config.completion_recovery_attempt_limit,
                    partial_content_present=bool(partial),
                )
    ...
    def _execute_action(self, action: ParsedAction, *, turn: int) -> str:
        ...
        if (
            ...
            if (
                self._stagnation_recovery_attempts
                >= self.config.stagnation_recovery_attempt_limit
            ):
                record_runtime_event(
                ...
                raise _StagnationRecoveryExhausted("STAGNATION_EXHAUSTED")
            self._stagnation_recovery_attempts += 1
            record_runtime_event(
                ...
                repeat_limit=limit,
                recovery_attempt=(self._stagnation_recovery_attempts),
                recovery_attempt_limit=(
                    self.config.stagnation_recovery_attempt_limit
                ),
            )
            generic_prompt = (
                ...
                "Use the current evidence to choose a different action that can make "
                "progress. If the task requirements are already satisfied, follow the "
                "completion protocol defined in the system prompt."
            )
            task_prompt = self.config.task_specific_stagnation_prompt.strip()
            if not task_prompt:
                return generic_prompt
            return (
                generic_prompt
                + "\n\nTask-specific recovery guidance:\n"
                + task_prompt
            )
        ...
        if not self.config.truncate_observations:
    ...
    def _build_context_length_exceeded_result(
        self,
        task: str,
        messages: list[Message],
        steps: list[AgentStep],
    ) -> AgentResult:
        """Fail explicitly when the complete request exceeds model context."""
        return self._build_terminal_failure_result(
            "CONTEXT_LENGTH_EXCEEDED", task, messages, steps
        )
    ...
    def _build_terminal_failure_result(
        ...
        """Return one explicit, non-retriable runtime-control failure."""
        self.state = AgentState.ERROR
        self._last_messages = messages
        self._last_steps = steps
    ...
    def _build_completion_length_exceeded_result(
        self,
        task: str,
        messages: list[Message],
        steps: list[AgentStep],
        ...
        return self._build_terminal_failure_result(
            "COMPLETION_LENGTH_EXCEEDED", task, messages, steps
        )

    def _build_format_error_exhausted_result(
        self,
        task: str,
        messages: list[Message],
        steps: list[AgentStep],
        *,
        ...
        record_runtime_event(
            "format_error_exhausted",
            turn=turn,
            instance_id=self._runtime_instance_id,
            protocol_sha256=self._runtime_protocol_sha256,
            consecutive_errors=consecutive_errors,
            error_limit=self.config.max_consecutive_format_errors,
        )
        return self._build_terminal_failure_result(
            "FORMAT_ERROR_EXHAUSTED", task, messages, steps
        )
    ...
    async def run_async(self, task: str) -> AgentResult:
        ...
        try:
            for turn in range(1, self.config.max_turns + 1):
                ...
                self.state = AgentState.THINKING
                response = await self._chat_with_completion_recovery(
                    messages, settings
                )
                ...
                if parse_result.is_format_error:
                    ...
                    if (
                        ...
                        return self._build_format_error_exhausted_result(
                            task,
                            messages,
                            steps,
                            turn=turn,
                            consecutive_errors=consecutive_format_errors,
                        )

                    # Add the generic format reminder and allow one bounded retry.
                ...
                except _StagnationRecoveryExhausted:
                    step.observation = "STAGNATION_EXHAUSTED"
                    steps.append(step)
                    if self.on_step:
                        self.on_step(step)
                    return self._build_terminal_failure_result(
                        "STAGNATION_EXHAUSTED", task, messages, steps
                    )
            ...
            return self._build_context_length_exceeded_result(task, messages, steps)

        except RequestCompletionLengthExceeded:
            return self._build_completion_length_exceeded_result(
                task, messages, steps
            )
    ...
    async def _run_continuation_loop_async(
        ...
        for turn in range(start_turn, self.config.max_turns + 1):
            ...
            # THINK: Get LLM response
            self.state = AgentState.THINKING
            response = await self._chat_with_completion_recovery(messages, settings)
