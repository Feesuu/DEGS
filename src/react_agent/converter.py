...
class ReActConverter:
    ...
    def parse_response(self, response: str) -> ParseResult:
        ...
        if json_end is None:
            # Qwen occasionally emits a complete nested action object but omits
            # only the final outer brace. Repair that single unambiguous transport
            # error; all other incomplete JSON remains fail-closed.
            if brace_count != 1:
                return ParseResult(
                    type=ParseResultType.FORMAT_ERROR,
                    error_message=FORMAT_ERROR_TEMPLATE,
                )
            json_str = action_text[json_start:].strip() + "}"
        else:
