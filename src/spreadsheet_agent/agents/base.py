...
class AgentContext:
    ...
    retrieval_id: str = ""  # Task-level identity shared by multiple workbook cases.
...
class BaseSpreadsheetAgent(ABC):
    ...
    def __init__(
        ...
        Args:
            ...
            stagnation_recovery_attempt_limit: Recovery prompts per stagnation episode
            max_completion_tokens: Per-request completion token limit
            completion_recovery_attempt_limit: Continuations after a length-limited reply
            max_consecutive_format_errors: Consecutive malformed replies allowed
            truncate_observations: Whether tool observations may be shortened
        """
        self.client = client
        self.max_turns = max_turns
        self.verbose = verbose
        self.stagnation_repeat_limit = stagnation_repeat_limit
        self.stagnation_recovery_attempt_limit = stagnation_recovery_attempt_limit
        self.max_completion_tokens = max_completion_tokens
        self.completion_recovery_attempt_limit = completion_recovery_attempt_limit
        self.max_consecutive_format_errors = max_consecutive_format_errors
        self.truncate_observations = truncate_observations
    ...
    def get_stagnation_recovery_prompt(self) -> str:
        """Return optional task-family guidance layered after generic recovery."""
        return ""
        ...
        if self._agent is None or self._working_dir != working_dir:
            ...
            config = AgentConfig(
                ...
                stagnation_recovery_attempt_limit=(
                    self.stagnation_recovery_attempt_limit
                ),
                task_specific_stagnation_prompt=self.get_stagnation_recovery_prompt(),
                max_completion_tokens=self.max_completion_tokens,
                completion_recovery_attempt_limit=(
                    self.completion_recovery_attempt_limit
                ),
                max_consecutive_format_errors=self.max_consecutive_format_errors,
                truncate_observations=self.truncate_observations,
            )
    ...
    def run(self, context: AgentContext) -> dict:
        ...
        try:
            ...
            output_exists = (
                os.path.isfile(context.output_file)
                and not os.path.islink(context.output_file)
            )
