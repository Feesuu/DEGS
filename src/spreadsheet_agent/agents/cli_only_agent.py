...
class CLIOnlyAgent(BaseSpreadsheetAgent):
    def __init__(
        ...
        super().__init__(
            ...
            truncate_observations=truncate_observations,
        )
    ...
    def get_stagnation_recovery_prompt(self) -> str:
        return (
            "Check whether `output.xlsx` exists and contains the requested workbook "
            "changes. If the output is already correct, signal "
            "`ACTION: TASK_COMPLETE`. Otherwise, use a different command that makes "
            "progress toward a correct `output.xlsx`."
        )
