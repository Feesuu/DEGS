...
- 每个数据集只使用其固定 train split 构图：SpreadsheetBench 为 `[0,200)`；Skill2Bench 为固定 seed-42 train-100。禁止任何 test/development outcome、gold、verifier result 或 Agent trace 进入构图与检索。
- source 只接纳 original-success，或最终有效 patch 加其对应 replay-success trace；不保存失败 replay history。
- 动态图按八个 train task 一批发布：SpreadsheetBench 为 25×8；Skill2Bench 为 12×8 加最后 4 个 task。已形成的 Canonical 组单调保留，不翻旧账拆组。
...
- Skill2Bench 每个 Step 独立执行同一 online retrieval；Step 顺序不构成因果依赖。WikiTQ/HiTab 只读匹配模型的冻结 SpreadsheetBench 图，不更新 source graph。
