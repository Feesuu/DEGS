...
def _resolve_within(root, *parts):
    resolved_root = Path(root).resolve()
    candidate = resolved_root.joinpath(*parts).resolve()
    try:
        candidate.relative_to(resolved_root)
    except ValueError as exc:
        raise ValueError(f"benchmark path escapes configured root: {candidate}") from exc
...
def find_spreadsheet_dir(data_path, instance):
    ...
    candidates = [
        _resolve_within(data_dir, "spreadsheet", spreadsheet_path),
        _resolve_within(data_dir, spreadsheet_path),
        _resolve_within(data_dir, "spreadsheet", instance_id),
        _resolve_within(data_dir, instance_id),
    ]
...
def find_output_dir(output_base, instance):
    ...
    candidates = [
        _resolve_within(output_base, spreadsheet_path),
        _resolve_within(output_base, instance_id),
    ]
